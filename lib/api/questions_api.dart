...
class QuestionsApi {
  ...
  static Future<ApiResult> createNewQuestion(QuestionsModel model) async {
    ...
    try {
      var result = await http.post(Uri.parse(_questionsEndPoint),
          headers: {
  ...
  static Future<List<QuestionsModel>> fetchQuestions() async {
    var result = await http.get(
        Uri.parse(_questionsEndPoint + 'unblocked'),
      headers: {
        "Content-type": "application/json",
        "Accept": "application/json"
      },
  ...
  static Future<List<QuestionsModel>> myQuestions(String userId) async {
    var result = await http.get(
        Uri.parse(_questionsEndPoint + 'unblocked/' + userId),
      headers: {
        "Content-type": "application/json",
        "Accept": "application/json"
      },
  ...
  static Future<List<AnswersModel>> fetchAnswers(String questionId) async {
    try {
      var result = await http.get(Uri.parse(_answersEndPoint + 'unblocked/$questionId'));
      if (result.statusCode == 200) {
  ...
  static Future<ApiResult> deleteQuestion(String questionId) async {
    try {
      var result = await http.delete(Uri.parse(_questionsEndPoint + questionId));
      if (result.statusCode == 200) {
        return ApiResult.successWithNoMessage();
      } else {
        print(result.body);
  ...
  static Future<ApiResult> deleteAnswer(String answerId) async {
    ...
    try {
      var result = await http.delete(Uri.parse(_answersEndPoint + answerId));
      if (result.statusCode == 200) {
        return ApiResult.successWithNoMessage();
      } else {
        print(result.body);
  ...
  static Future<int> getCountOfAnswersByUser(String userId) async {
    try {
      ...
      var result =
          await http.get(Uri.parse(_answersEndPoint + 'user/${AppConfig.userId}'));
