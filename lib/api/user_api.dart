...
class UserApi {
  ...
  static Future<UserModel> createUser({
    ...
    try {
      var result = await http.post(Uri.parse(_endPoint),
          headers: {
  ...
  static Future<UserModel> loginUser({
    ...
    try {
      var result = await http.post(Uri.parse(_endPoint + 'login'),
          headers: {
            "Content-type": "application/json",
            "Accept": "application/json"
          },
  ...
  static Future<UserModel> getUser({
    ...
    var result = await http.get(Uri.parse(_endPoint + uid));
    if (result.statusCode == 200) {
      return UserModel.fromMap(json.decode(result.body));
    } else {
      return null;
  ...
  static Future<List<UserModel>> getAllAdvocates(String id) async {
    var result = await http.get(Uri.parse(_endPoint + 'allAdvocates'));
    print(result);
  ...
  static Future<UserModel> getUserByEmail(String email) async {
    var result = await http.get(Uri.parse(_endPoint));
    if (result.statusCode == 200) {
      List<dynamic> data = json.decode(result.body);
      UserModel user ;
      data.forEach((element) {
        if (element['email'] == email) user= UserModel.fromMap(element);
      });
      return user;
    } else {
      return null;
  ...
  static Future<UserModel> getUserByName(String name) async {
    try {
      var result = await http.get(Uri.parse(_endPoint+'name'));
      print(result);
      if (result.statusCode == 200) {
        UserModel user;
  ...
  static Future<List<UserModel>> getAllAdvocatesWithFilter(
    ...
    if (data?.separator != null) {
      ...
      queryParameters['advocateName'] = data.advocateName.toString();
    }

    String query = Uri(queryParameters: queryParameters).query;

    var result = await http.get(Uri.parse(_endPoint + 'allAdvocates?$query'));
    if (result.statusCode == 200) {
      List<dynamic> data = json.decode(result.body);
      List<UserModel> advocates = [];
      data.forEach((element) {
  ...
  static Future<ApiResult> updateAdvocateDetails(
    ...
    try {
      var result = await http.patch(Uri.parse(_endPoint + userId),
          headers: {
            "Content-type": "application/json",
            "Accept": "application/json"
          },
  ...
  static Future<ApiResult> updatePhoto(
      String userId,
      String url,
      ) async {
    try {
  ...
  //   return await YlcCollectionRef.usersCollection
  //       .doc(userId)
  //       .update({'token': token});
  // }
}
